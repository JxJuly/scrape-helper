...
import { Button, Space, Table, Typography, Divider, Image } from 'antd';

import { TableCellText, TableCellTags } from '../components';
...
const ScrapeButton: React.FC<{ filePath: string }> = ({ filePath }) => {
  ...
  const handleScrape = async () => {
    try {
      setLoading(true);
      await window.scrapeApi.animate(filePath);
    } finally {
      setLoading(false);
    }
  };
...
const colums: TableColumnsType = [
  {
    ...
    width: 240,
    render: (value) => <TableCellText text={value} />,
  },
  {
    title: '封面',
    dataIndex: 'poster',
  ...
  },
  {
    ...
    render: (metaData, info) => {
      ...
      return <TableCellText text={title || '没有数据嗷~'} />;
    },
  },
  {
    title: '简介',
    ...
    render: (metaData, info) => {
      ...
      return <TableCellText text={description || '没有数据嗷~'} />;
    },
  },
  {
    title: '评分',
  ...
  },
  {
    ...
    render: (metaData, info) => {
      ...
      return <TableCellTags tags={tags || []} />;
    },
  },
  {
    title: '工作室',
    ...
    render: (metaData, info) => {
      ...
      return <TableCellTags tags={studios || []} />;
    },
  },
  {
    title: '操作',
